...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"chats" | "reviews" | "sales">("chats");
                ...
                {sales.map((order, idx) => (
                  <div key={idx} className="border-b pb-3 mb-3">
                    ...
                    {order.status === "failed" && order.error && (
                      <div className="text-xs text-red-500 mt-1">Error: {order.error}</div>
                    )}
                  </div>
                ))}
              </div>
